...
const CROSS_ORIGIN_FORM_EXEMPT_PREFIXES = ['/api/quick-capture/'];
...
export function isForbiddenCrossOriginForm(request: Request, url: URL): boolean {
	if (
		CROSS_ORIGIN_FORM_EXEMPT_PATHS.has(url.pathname) ||
		CROSS_ORIGIN_FORM_EXEMPT_PREFIXES.some((prefix) => url.pathname.startsWith(prefix))
	) return false;
